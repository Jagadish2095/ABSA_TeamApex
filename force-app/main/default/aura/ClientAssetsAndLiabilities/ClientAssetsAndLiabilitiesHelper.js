({
    handleInit : function(component) {
        ...
        if(accountId){
            var action = component.get("c.getAssetsLiabilities");
                ...
                if (state === "SUCCESS") {
                    ...
                    if(assetsAndLiabilities){
                        ...
                        if(assetsAndLiabilities[0]){
                            component.set("v.clientName", assetsAndLiabilities[0].FinServ__PrimaryOwner__r.Name);
                            component.set("v.outcomePositive", assetsAndLiabilities[0].FinServ__PrimaryOwner__r.Outcome_positive_Y_N__c);
                            component.set("v.deedsOfficeSearchAvailable", assetsAndLiabilities[0].FinServ__PrimaryOwner__r.Deeds_office_search_available_Y_N__c);
                            component.set("v.dateDeedsOfficeSearch", assetsAndLiabilities[0].FinServ__PrimaryOwner__r.Date_deeds_office_search__c);
                            component.set("v.asAtDate", assetsAndLiabilities[0].FinServ__PrimaryOwner__r.As_at_date__c);
                            
                            if(assetsAndLiabilities[0].FinServ__PrimaryOwner__r.Id_Number__pc){
                                component.set("v.idNumber", assetsAndLiabilities[0].FinServ__PrimaryOwner__r.Id_Number__pc);
                            } else if(assetsAndLiabilities[0].FinServ__PrimaryOwner__r.Passport_Number__pc){
                                component.set("v.idNumber", assetsAndLiabilities[0].FinServ__PrimaryOwner__r.Passport_Number__pc);
                            }
                        }
                    }
                    ...
                    if (errors) {
                        ...
                        this.showToast("Error!","Assets & Liabilities unknown error","error");
                    }
                }
    ...
    handleSubmit : function(component){
        ...
        if(!asAtDateError && !dateDeedsError){
            component.set("v.showSpinner", true);
            var action = component.get("c.updateAssetsLiabilities");
            
            action.setParams({
                "accAssetsLiabilities" : assetsAndLiabilities,
    ...
    updateAssetsBranchTotal : function(component) {
        ...
        if(assetsAndLiabilities){
            for (var i = 0; i < assetsAndLiabilities.length; i++){
                if(assetsAndLiabilities[i].RecordType.Name == 'Asset'){
                    if(assetsAndLiabilities[i].FinServ__Amount__c){
                        assetsBranchTotal += this.checkForNaN(assetsAndLiabilities[i].FinServ__Amount__c);
                    }
                }
            }
        }
    ...
    updateLiabilitiesBranchTotal : function(component) {
        ...
        if(assetsAndLiabilities){
            for (var i = 0; i < assetsAndLiabilities.length; i++){
                if(assetsAndLiabilities[i].RecordType.Name == 'Liability'){
                    if(assetsAndLiabilities[i].FinServ__Amount__c){
                        liabilitiesBranchTotal += this.checkForNaN(assetsAndLiabilities[i].FinServ__Amount__c);
                    }
                }
            }
        }
        ...
        if(assetsAndLiabilities){
            for (var i = 0; i < assetsAndLiabilities.length; i++){
                if(assetsAndLiabilities[i].RecordType.Name == 'Fixed Property & Bond' && 
                   (assetsAndLiabilities[i].FinServ__AssetsAndLiabilitiesType__c == 'Fixed Properties Bonded' || 
                  	assetsAndLiabilities[i].FinServ__AssetsAndLiabilitiesType__c == 'Fixed Properties Unbonded' || 
                  	assetsAndLiabilities[i].FinServ__AssetsAndLiabilitiesType__c == 'Fixed Properties Informal' )){
                    if(assetsAndLiabilities[i].Amount_Owners_Valuation__c){
            ...
            for (var i = 0; i < assetsAndLiabilities.length; i++){
                if(assetsAndLiabilities[i].RecordType.Name == 'Fixed Property & Bond' && 
                   (assetsAndLiabilities[i].FinServ__AssetsAndLiabilitiesType__c == 'Fixed Properties Bonded' || 
                  	...
                  	assetsAndLiabilities[i].FinServ__AssetsAndLiabilitiesType__c == 'Fixed Properties Informal' )){
                    if(assetsAndLiabilities[i].FinServ__Amount__c){
                        fixedPropertyBranchTotal += this.checkForNaN(assetsAndLiabilities[i].FinServ__Amount__c);
                    }
                }
            }
        }
