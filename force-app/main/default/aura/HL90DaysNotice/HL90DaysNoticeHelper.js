({
	...
	getAccountInformationNotice: function (component) {
		var noticeStartDate = new Date();
		var noticeEndDate = new Date();
		noticeEndDate.setDate(noticeEndDate.getDate() + 90);

		var formatNoticeStartDate = $A.localizationService.formatDate(noticeStartDate, "MMMM dd, yyyy");
		var formatNoticeEndDate = $A.localizationService.formatDate(noticeEndDate, "MMMM dd, yyyy");
		var validityPeriod = formatNoticeStartDate + " to " + formatNoticeEndDate;

		let emailBodyMap = new Map();
		emailBodyMap["[#Validity Period]"] = validityPeriod;

		component.set("v.emailBodyMappingToFlow", JSON.stringify(emailBodyMap));
	...
	updateAccountInformationNotice: function (component) {
		...
		action.setCallback(this, function (response) {
			...
			if (state === "SUCCESS") {
				...
				if (responseData.startsWith("Error: ")) {
					...
					this.fireToastEvent("Success", "The 90 day hold has successfully been loaded", "Success");
					component.find("btnSubmit").set("v.disabled", true);
					this.createPropertyHubCase(component);
				}
			} else if (state === "ERROR") {
				var errors = response.getError();
				component.set("v.errorMessage", "updateAccountInformationNotice error: " + JSON.stringify(errors[0].message));
			} else {
				component.set("v.errorMessage", "updateAccountInformationNotice unexpected error occurred, state returned: " + state);
			}
			this.hideSpinner(component);
		});
		$A.enqueueAction(action);
