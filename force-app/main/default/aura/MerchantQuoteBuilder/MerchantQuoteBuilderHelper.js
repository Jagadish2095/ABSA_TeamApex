({
        ...
        action.setCallback(this, function(response) {
            ...
            if (component.isValid() && state === "SUCCESS") {
                ...
                if(respBean != null && respBean.productName != null){
                    ...
                    this.createUI(component, helper, respBean);
    ...
    createUI: function(component, helper, respBean) {

        component.set("v.pricingBean", respBean);
        var finalArray = [];
        var arrayAuraIdsToBeValidated = [];
        var layout = ["lightning:layout", {"multipleRows": true}];
        var layoutItemSize6 = ["lightning:layoutItem", {"size": "6","padding": "around-small"}];
        ...
        respBean.fieldVisibilityMdtList.forEach(function (item) {
            ...
            if(!$A.util.isEmpty(additionalAttributes.picklistOptionsPath)){
            ...
            if(item.Component_Type__c == "lightning:select" && picklistOptions != null){
                //Add '--- None ---'
            ...
            //Add Layout Item
            finalArray.push(layoutItemSize6);
        });
        ...
        $A.createComponents(finalArray,function(components, status, errorMessage){
            ...
            if (status === "SUCCESS") {
                ...
                var labelList = [];
                var labelTextList = [];
                component.set("v.body", []);//clear v.body
                ...
                //Set Component Body
                component.set("v.body", layoutCmp);
    ...
    mapDataToUIFields: function(component, helper, respBean){
        ...
        respBean.fieldVisibilityMdtList.forEach(function (item) {
            ...
            if(!$A.util.isEmpty(additionalAttributes.valueReadPath)){
                value = helper.getDeep(respBean, additionalAttributes.valueReadPath);
            }
            //Set the value at the Map Key
            metadataNameToValueMap[item.DeveloperName] = value;
        ...
        component.set('v.allVASProducts', respBean.vasProductDataList);
        //Toggle if read only
        if(respBean.quoteGenerationDate != null){
            //this.formReadOnly(component, true);
            component.set('v.isFormReadOnly', true);
        } else {
            //this.formReadOnly(component, false);
    ...
    mapFieldsToDataTable: function(component, helper, respBean) {
        ...
        var data = [{
            ...
            merchantService: respBean.productName,
    ...
    generateMerchantQuote: function(component, helper){
        ...
        action.setCallback(this, function(response) {
            ...
            if (component.isValid() && state === "SUCCESS") {
                ...
                this.hideSpinner(component);
                var errors = response.getError();
                this.fireToast("Error", "Apex error: [" + JSON.stringify(errors) + "].", "error");
                //this.formReadOnly(component, false);
                component.set('v.isFormReadOnly', false);
            } else {
                this.hideSpinner(component);
...
// end Tinashe bug to display the contract duration
    ...
    reviseQuote: function(component, helper) {
        ...
        //this.populateIsNegotiableAttributes(component, component.get('v.pricingBean'));
    },
            ...
            if (component.isValid() && state === "SUCCESS") {
                ...
                if (error) {
                    ...
                    if ($A.util.isEmpty(applicationId) || $A.util.isEmpty(applicationProductMerchantId)) {
                        ...
                        //$A.get('e.force:refreshView').fire(); //Refresh the page once record is Created Successfully
    ...
    //W-005082 JQUEV
    /*closeRejectQuoteModal : function(component){
        var cmpTarget = component.find('lossReasonModal');
        var cmpBack = component.find('Modalbackdrop');
    ...
    setColumns: function(component, actions) {
        component.set('v.columns', [
            ...
            {label: 'Merchant Service', fieldName: 'merchantService', type: 'text'},
