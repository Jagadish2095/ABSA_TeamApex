    ...
    handleUnlimitedSubmit : function(component, event, helper) {
        ...
        for (var i=0; i< itemsToPass.length; i++)
        {
            ...
            if(item.Guarantor_name__c=='' || item.Guarantor_name__c==undefined || item.Guarantor_registration_number__c=='' || item.Guarantor_registration_number__c==undefined ||
              item.Session_of_claims_and_loans_accounts__c==''  || item.CPA_document_version__c==''  || item.Include_address__c=='' || item.Include_address__c==undefined){ //|| item.Session_of_claims_and_loans_accounts__c==undefined || item.CPA_document_version__c==undefined
                checkStatus = true;
            }
            if(item.To_be_released__c=='Yes'){
                 if(item.When_to_release_months__c=='' || item.When_to_release_months__c==undefined || item.Release_condition__c=='' || item.Release_condition__c==undefined){
                     checkStatus = true;}
            }
         /*   if(item.Include_address__c=='Yes'){
                
                 if(item.Guarantor_email_address_for_notices__c=='' || item.Guarantor_email_address_for_notices__c==undefined || item.Address_line_1__c=='' || item.Address_line_1__c==undefined || item.Suburb__c=='' || item.Suburb__c==undefined || item.City__c=='' || item.City__c==undefined || item.Postal_Code__c=='' || item.Postal_Code__c==undefined || item.Country__c=='' || item.Country__c==undefined){
                     checkStatus = true;}
            }*/
        }
        if(checkStatus ==true){
