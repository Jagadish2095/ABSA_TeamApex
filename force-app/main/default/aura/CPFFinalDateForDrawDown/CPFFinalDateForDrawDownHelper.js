        ...
        action.setCallback(this, function (response) {
            ...
            if (state === "SUCCESS") {
                ...
                if(appProductCPFId){
                    ...
                    this.showToast("Error!", "Final Date For Draw Down: No Application Product CPF Record Found", "error");
                }
            } else {
                ...
                if (errors) {
                    if (errors[0] && errors[0].message) {
                        if(String(errors[0].message).includes('List has no rows for assignment to SObject')){
                            this.showToast("Error!", "Final Date For Draw Down: Please capture a product", "error");
                        } else {
                            this.showToast("Error!", "Final Date For Draw Down: " + errors[0].message, "error");
                        }
                    }
                } else {
                    this.showToast("Error!", "Final Date For Draw Down unknown error", "error");
                }
            }
        });
