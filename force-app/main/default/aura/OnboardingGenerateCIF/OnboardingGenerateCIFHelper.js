...
****************@ Description: Account and RelatedPartyDetails*********/
                ...
                getOpportunityAction.setCallback(this, function(response) {
                    ...
                    if (component.isValid() && state === "SUCCESS") {
                        ...
                        if(oppDetails != null) {
                            ...
                            console.log('oppDetails FraudRiskAlert__c: ' + oppDetails.FraudRiskAlert__c);
                            console.log('oppDetails FraudRiskActionTaken__c: ' + oppDetails.FraudRiskActionTaken__c);
                            ...
                            // && oppDetails.FraudRiskAlert__c!=undefined && oppDetails.FraudRiskActionTaken__c!='Known Fraud'
                            ...
                            //Hide Generate CIF if Fraus is Suspet or High
                             if (oppDetails.FraudRiskAlert__c == 'Clean') {
                                 //component.set("v.showGenerateCIFButton", true);
                             } else {
  ...
  createCIFHelper : function(component, event, helper) {
        ...
        this.showSpinner(component);
    	var action = component.get("c.callToCreateCIF"); 
         action.setParams({
            "oppid": component.get("v.recordId"),
            "accRecordTypeName" : component.get("v.account.RecordType.DeveloperName")
        });
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state == "SUCCESS") {
               ...
               if(respmsg.errormsglist!=null){
                ...
                if(respmsg.cifoutput != null && respmsg.cifoutput != undefined){
                     var processType = component.get("v.opportunityRecord.Process_Type__c")
                     if(processType == 'Surety Onboarding') {
                         this.updateCIFwithSuretyIndicator(component, event, helper);
                     }
                     
                    //this.confirmDocumentReceiptHelper(component, event, helper);
                    this.getDocumentsCasaTiersHelper(component, event, helper);
                    component.set("v.cifKey",respmsg.cifoutput);
                     component.set("v.cifopt",true);
                     var ee=component.get("v.cifopt");
                     console.log('ee'+ee);
                ...
                toastEvent.setParams({
                    "type": "error",
                    "title": "",
                    "message": respmsg.cifoutputerror
                });
                toastEvent.fire();
                }
            } else{
            ...
            if (state === "SUCCESS") {
                ...
                if(eddRequiredDocuments.length != 3){
                    ...
                    toastEvent.setParams({
                        "type": "error",
                        "title": "",
                        "message": "Please upload EDD Documents: Adverse Media Report, Senior Management Sign off and Source of Funds/Wealth"
                    });
                    toastEvent.fire();
                    this.hideSpinner(component);
                }
  ...
  // if any Related party status is not 'Approved' Stop user to do Risk Profiling and make relPartyInvalidStatusFlag is true
  CheckRelatedPartyCasaStatus : function(component, event, helper) {
    ...
    //W-006880 -Anka Ganta -2020-10-27
    SourceofWealthValidation: function (component, event, helper) {
                    this.showSpinner(component);
				    var caseRec = component.get("v.opportunityRecord.Case__c");
					var caseStatus = component.get("v.opportunityRecord.Case__r.Status");
                    var SOW = component.get("v.opportunityRecord2.Source_of_Wealth_Capitalization__c");
                    var SOWComments = component.get("v.opportunityRecord2.Comments_on_Explanation_given_for_Source__c");
                    var Resolvedlabel = $A.get("$Label.c.Resolved");
                    console.log("SOW "+ SOW);
                    console.log("SOWComments "+ SOWComments);
                    if($A.util.isUndefinedOrNull(caseRec) || caseStatus == Resolvedlabel){
                        this.createCIFHelper(component, event, helper);
                    }
                    else if ($A.util.isUndefinedOrNull(SOW)  || $A.util.isUndefinedOrNull(SOWComments)){
                        debugger;
                        var toastEvent = $A.get("e.force:showToast");
                        toastEvent.setParams({
                            "type": "error",
                            "title": "",
                            "message": "Please complete SOURCE OF WEALTH CAPITALIZATION SECTION on Opportunity to proceed further"
                        });
                        toastEvent.fire();
                    }
            this.hideSpinner(component);
    ...
    //TdB - Surety Onboarding
     updateCIFwithSuretyIndicator : function(component, event, helper) {
        this.showSpinner(component);
    	var action = component.get("c.updateClientDetailsCIF"); 
         action.setParams({
            "oppId": component.get("v.recordId"),
            "accRecordTypeName" : component.get("v.account.RecordType.DeveloperName")
        });
        ...
        $A.enqueueAction(action);
         ...
         //Anka Ganta - Remediate Existing Customer Onboarding
     updateCIFRemediateHelper : function(component, event, helper) {
        this.showSpinner(component);
    	var action = component.get("c.updateCIFRemediate"); 
         action.setParams({
            "oppId": component.get("v.recordId"),
            "accRecordTypeName" : component.get("v.account.RecordType.DeveloperName")
        });
        action.setCallback(this, function(response) {
            ...
            if (state == "SUCCESS") {
               ...
               if(respmsg.errormsglist!=null && respmsg.errormsglist.length > 0){
                var errormsgs='';
                    var i;
                    for (i in respmsg.errormsglist) {
                       errormsgs += respmsg.errormsglist[i] + ";";
                ...
                toastEvent.fire();
                 ...
                 /*if(respmsg.cifoutput!=null){
                    ...
                    "type": "success",
                    "title": "Success!",
                    "message": "Customer Details has been update to client CIF successfully"
                });
                toastEvent.fire();
                }
            } else{
