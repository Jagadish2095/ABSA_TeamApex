({
	SecurityCessionOfSchemesHelper : function(component, event, helper) {
     var pageSize = component.get("v.pageSize");
     component.set('v.isHide', true);
     var action = component.get("c.getCessionOfSchemes");
     ...
     console.log("Account " + accountId);
        ...
        action.setCallback(this, function(response) {
            ...
            console.log("Get Cession of Schemes Response **********************************************************" + JSON.stringify(response));
            if (state === "SUCCESS" && response != null || response != '') {
                var storeResponse = JSON.parse(response.getReturnValue());
                console.log("Inside Method 1" + JSON.stringify(storeResponse));
                
                if(storeResponse != null){
                    console.log("List of Bonds ****** " + JSON.stringify(storeResponse));
               			 // if storeResponse size is equal 0 ,display No Result Found... message on screen. 
                	if (storeResponse.length > 0) {
                    	var bondsList = [];
                		var count = 0;
                
                		for(var key = 0 ; key < 2 && key< storeResponse.length ; key++){ 
                	...
                	component.set("v.errorMessage","No Securities Found");
