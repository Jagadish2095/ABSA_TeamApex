({
        ...
        if(securitiessections =='New'){
            for (var i=0; i< itemsToPass.length; i++)
            {
                ...
                if(item.Suretyship_name__c=='' || item.Suretyship_name__c==undefined || item.Suretyship_registration_number__c=='' || item.Suretyship_registration_number__c==undefined ||
                   item.Session_of_claims_and_loans_accounts__c=='' || item.CPA_document_version__c=='' || item.Amount__c=='' || item.Amount__c==undefined || item.Include_address__c=='' || item.Include_address__c==undefined){ // || item.Session_of_claims_and_loans_accounts__c==undefined || item.Date_registered__c==undefined 
                    checkStatus = true;
                }
            ...
            if(checkStatus ==true){
                ...
                helper.InsertLimitedforNewCpf(component, event, helper);
            }
        }
    },
    ...
    closeModel: function(component, event, helper) {
     ...
     handleChange: function (component, event,helper) {
        ...
        console.log('changeValue in new '+changeValue);
        helper.InsertLimitedforNewCpf(component, event, helper,changeValue);
        component.set("v.isModalOpen", false);
