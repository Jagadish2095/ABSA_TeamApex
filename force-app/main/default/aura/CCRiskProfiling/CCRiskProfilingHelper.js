({
    riskProfiling: function(component, helper) {
        return new Promise(function(resolve, reject) {
            component.set("v.showSpinner", true);
            var recordId = component.get("v.recordId");
            var action = component.get("c.callRiskProfiling");
            action.setParams({
                "oppId" : recordId
            });
   ...
   handleError: function (component, error) {
    ...
    handleSuccess: function (component, showMou) {
        var mouMsg = "We need to conduct further controls, expect the delay in the processing your application.";
        var isCalledFromFlow = component.get("v.isCalledFromFlow");
        component.set("v.showSuccess", true);
        if (isCalledFromFlow) {
            if (showMou) {
                component.find('branchFlowFooter').set('v.heading', "Please Note:");
                component.find('branchFlowFooter').set('v.message', mouMsg);
                component.find('branchFlowFooter').set('v.showDialog', true);
            }
            ...
            if (showMou) {
                component.set("v.errorDisplay", true);
                component.set("v.errorMessage", mouMsg);
            }
